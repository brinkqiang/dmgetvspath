...
template <typename Iterator, typename T>
inline bool string_to_type_converter( const std::pair<Iterator, Iterator>&
                                      range, T& t ) {
...
template <typename T, typename Iterator>
inline T string_to_type_converter( const std::pair<Iterator, Iterator>&
                                   range ) {
...
inline bool string_to_type_converter( const std::string& s, T& t ) {
    return string_to_type_converter<const char*, T>( s.data(), s.data() + s.size(),
            t );
}
...
inline adapter<typename Sequence<T, Allocator>::iterator> type(
    ...
    return adapter<typename Sequence<T, Allocator>::iterator>( seq.begin(),
            seq.end() );
}
...
struct single_delimiter_predicate {
  ...
  private:
    ...
    single_delimiter_predicate<T>& operator=( const
            single_delimiter_predicate<T>& );
    const T delimiter_;
};
...
inline void remove_consecutives_inplace( const char* rem_chars,
        std::string& s ) {
    if ( s.empty() ) {
        return;
    }
...
void remove_empty_strings( Sequence<std::string, Allocator>& seq ) {
    ...
    seq.erase( std::remove_if( seq.begin(), seq.end(), is_empty::check ),
               seq.end() );
}
...
void remove_empty_strings( std::set<std::string, Comparator, Allocator>& set ) {
    ...
    typename std::set<std::string, Comparator, Allocator>::iterator itr =
        set.begin();
...
template <typename Comparator, typename Allocator>
void remove_empty_strings( std::multiset<std::string, Comparator, Allocator>&
                           set ) {
    struct is_empty {
        static inline bool check( const std::string& s ) {
            return s.empty();
...
inline bool match( const Iterator pattern_begin,
    ...
    */
    ...
    while ( ( p_itr != pattern_end ) && ( zero_or_more == ( *p_itr ) ) ) {
        ++p_itr;
    }
...
inline std::size_t ifind( const std::string& pattern,
...
inline bool begins_with( const InputIterator pattern_begin,
    ...
    if ( std::distance( pattern_begin, pattern_end ) <= std::distance( begin,
            end ) ) {
...
inline bool ibegins_with( const std::string& pattern,
                          const std::string& data ) {
    if ( pattern.size() <= data.size() ) {
...
inline std::size_t index_of( const std::string& pattern,
                             const std::string& data ) {
    if ( pattern.empty() ) {
        return std::string::npos;
    }
    ...
    const char* itr = std::search( data.data(),
                                   ...
                                   pattern.data(),
                                   pattern.data() + pattern.size() );
    return ( ( data.data() + data.size() ) == itr ) ? std::string::npos :
...
class range_to_type_back_inserter_iterator : public
  ...
  public:
    ...
    range_to_type_back_inserter_iterator( const
                                          range_to_type_back_inserter_iterator& it )
...
class range_to_type_inserter_iterator : public
  ...
  public:
    ...
    inline range_to_type_inserter_iterator& operator=( const
            range_to_type_inserter_iterator& it ) {
        if ( this != &it ) {
            this->set_ = it.set_;
        }
...
class range_to_type_push_inserter_iterator : public
  ...
  public:
    ...
    range_to_type_push_inserter_iterator( const
                                          range_to_type_push_inserter_iterator& it )
...
class back_inserter_with_valuetype_iterator : public
  ...
  public:
    ...
    back_inserter_with_valuetype_iterator( const
                                           back_inserter_with_valuetype_iterator& it )
        : sequence_( it.sequence_ )
    {}
...
template <typename Set>
inline inserter_with_valuetype_iterator<Set> inserter_with_valuetype(
    Set& set_ ) {
...
class range_to_ptr_type_iterator : public
  ...
  public:
    ...
    template <typename Iterator>
    inline range_to_ptr_type_iterator& operator=( const
            std::pair<Iterator, Iterator>& r ) {
        value_type t = value_type();
...
class functional_inserter_iterator : public
  ...
  public:
    ...
    inline functional_inserter_iterator& operator=( const
            functional_inserter_iterator& it ) {
        if ( this != &it ) {
...
inline std::size_t split( const DelimiterPredicate& delimiter,
    ...
    const bool compress_delimiters = split_options::perform_compress_delimiters(
                                         split_option );
    const bool include_1st_delimiter = split_options::perform_include_1st_delimiter(
                                           split_option );
...
inline std::size_t split( const char* delimiters,
    ...
    if ( 1 == details::strnlength( delimiters, 256 ) )
        return split( single_delimiter_predicate<std::string::value_type>
                      ( delimiters[0] ),
                      range.first, range.second,
                      out,
                      split_option );
...
inline offset_predicate<10> offsets( const int& v1, const int& v2,
                                     const int& v3,
                                     const int& v4, const int& v5, const int& v6,
                                     const int& v7, const int& v8, const int& v9,
...
template <typename T,
          typename Comparator,
          typename Allocator>
inline T min_of_cont( const std::multiset<T, Comparator, Allocator>&
                      multiset ) {
...
template <typename T,
          typename Comparator,
          typename Allocator>
inline T max_of_cont( const std::multiset<T, Comparator, Allocator>&
                      multiset ) {
    return ( *multiset.rbegin() );
}

template <typename InputIterator>
inline void min_max_of_range( const InputIterator begin,
                              const InputIterator end,
...
inline const char* first_non_repeated_char( const char* begin,
        const char* end ) {
    static const std::size_t lut_size = 256;
    unsigned long long int lut[lut_size];

    std::fill_n( lut, lut_size,
...
inline std::size_t first_non_repeated_char( const std::string& str ) {
    ...
    const char* itr = first_non_repeated_char( str.data(),
                      str.data() + str.size() );
...
inline void convert_bin_to_hex( const char* begin, const char* end,
                                char* out ) {
...
inline bool convert_hex_to_bin( const std::string& hex_data,
                                std::string& output ) {
...
inline std::size_t convert_bin_to_base64( const unsigned char* begin,
    ...
    return static_cast<std::size_t>( ( length / 3 ) * 4 ) + ( (
                length % 3 ) > 0 ? 4 : 0 );
}
...
inline void convert_to_printable_chars( unsigned char* begin,
                                        unsigned char* end ) {
...
inline void convert_to_uppercase( std::string& str ) {
    convert_to_uppercase( reinterpret_cast<unsigned char*>( const_cast<char*>
                          ( str.data() ) ),
                          reinterpret_cast<unsigned char*>( const_cast<char*>( str.data() +
                                  str.size() ) ) );
}
...
inline void convert_to_lowercase( std::string& str ) {
    convert_to_lowercase( reinterpret_cast<unsigned char*>( const_cast<char*>
                          ( str.data() ) ),
                          reinterpret_cast<unsigned char*>( const_cast<char*>( str.data() +
                                  str.size() ) ) );
}
...
inline std::string as_secondcase(const std::string& str) {
    std::string result = str;
    std::transform(result.begin(), result.end(), result.begin(), ::tolower);
    if (result.size() >= 2)
    {
...
inline bool twoway_bitwise_interleave( const unsigned char* begin1,
    ...
    while ( end1 != itr1 ) {
        *( reinterpret_cast<unsigned short*>( out ) )  = ( interleave_table[*
                ( itr2++ )] << 1 );
...
inline bool twoway_bitwise_interleave( const char* begin1, const char* end1,
                                       ...
                                       char* out ) {
    return twoway_bitwise_interleave( reinterpret_cast<const unsigned char*>
                                      ( begin1 ),
...
class token_grid {
  ...
  private:
    ...
    struct store {
        ...
        inline range_t operator()( const std::size_t& col,
        ...
        inline bool remove_row_range( const std::size_t& r0, const std::size_t& r1 ) {
            ...
            token_list_t::iterator remove_end = token_list.begin() + rr0.first +
                                                number_of_tokens;
            token_list.erase( remove_begin, remove_end );
            row_index.erase( row_index.begin() + r0,
                             row_index.begin() + r0 + ( r1 - r0 + 1 ) );
  ...
  public:
    ...
    inline row_range_t range( std::size_t lower_bound,
        ...
        if ( upper_bound == std::numeric_limits<std::size_t>::max() ) {
            ...
            return row_range_t( std::numeric_limits<std::size_t>::max(),
    ...
    class row_type {
      ...
      public:
        ...
        inline std::string as_string() const {
            std::string result;
            result.reserve( std::distance( begin_->first,
                                           ( begin_ + ( size_ - 1 ) )->second ) );
            token_list_t::const_iterator itr = begin_;
        ...
        template <typename T,
                  typename Container,
                  typename Comparator>
        inline bool parse( std::priority_queue<T, Container, Comparator>&
        ...
        template <typename T,
                  typename Comparator,
                  typename Allocator>
        inline void parse_checked( std::multiset<T, Comparator, Allocator>& multiset )
        const {
        ...
        template <typename Function>
        inline std::size_t for_each_column( const col_range_t& range,
                                            Function f ) const {
            if ( !validate_column_range( range ) ) {
                return false;
            }
    ...
    token_grid( const char* input_buffer,
                ...
                const token_grid::options& options )
        : file_name_( "" ),
          buffer_( reinterpret_cast<unsigned char*>( const_cast<char*>
                   ( input_buffer ) ) ),
          buffer_size_( input_buffer_size ),
          min_column_count_( 0 ),
          max_column_count_( 0 ),
    ...
    inline bool extract_column_checked( const row_range_t& row_range,
        ...
        for ( std::size_t i = row_range.first; i < row_range.second; ++i ) {
            ...
            if ( index < dsv_index_.token_count( row ) ) {
                ...
                process_token_checked( *( dsv_index_.token_list.begin() +
                                          ( row.first + index ) ), out );
            }
        }
    ...
    inline bool extract_column( const row_range_t& row_range,
        ...
        for ( std::size_t i = row_range.first; i < row_range.second; ++i ) {
            ...
            if ( index < dsv_index_.token_count( row ) ) {
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index ) ),
                               out );
            }
        }
    ...
    inline bool extract_column( const row_range_t& row_range,
        ...
        for ( std::size_t i = row_range.first; i < row_range.second; ++i ) {
            ...
            if ( max_index < dsv_index_.token_count( row ) ) {
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index0 ) ),
                               out0 );
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index1 ) ),
                               out1 );
            }
        }
    ...
    inline bool extract_column( const row_range_t& row_range,
        ...
        for ( std::size_t i = row_range.first; i < row_range.second; ++i ) {
            ...
            if ( max_index < dsv_index_.token_count( row ) ) {
                ...
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index1 ) ),
                               out1 );
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index2 ) ),
                               out2 );
            }
        }
    ...
    inline bool extract_column( const row_range_t& row_range,
        ...
        for ( std::size_t i = row_range.first; i < row_range.second; ++i ) {
            ...
            if ( max_index < dsv_index_.token_count( row ) ) {
                ...
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index2 ) ),
                               out2 );
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index3 ) ),
                               out3 );
            }
        }
    ...
    inline bool extract_column( const row_range_t& row_range,
        ...
        for ( std::size_t i = row_range.first; i < row_range.second; ++i ) {
            ...
            if ( max_index < dsv_index_.token_count( row ) ) {
                ...
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index3 ) ),
                               out3 );
                process_token( *( dsv_index_.token_list.begin() + ( row.first + index4 ) ),
                               out4 );
            }
        }
    ...
    inline std::size_t accumulate_row( const std::size_t& row, T& result ) const {
        ...
        while ( end != itr ) {
            if ( string_to_type_converter( ( *itr ).first, ( *itr ).second,
                                           current_value ) ) {
    ...
    template <typename Function>
    inline std::size_t for_each_row( const row_range_t& row_range,
                                     Function f ) const {
        if ( row_range_invalid( row_range ) ) {
            return 0;
        }
        ...
        if ( !options_.support_dquotes ) {
            ...
            strtk::split( text_newline_predicate,
                          ...
                          strtk::functional_inserter(
                              row_processor<multiple_char_delimiter_predicate>( dsv_index_, token_predicate,
            ...
            strtk::split( text_newline_predicate,
                          buffer_, buffer_ + buffer_size_,
                          strtk::functional_inserter(
                              row_processor<double_quotes_predicate>( dsv_index_, token_predicate_dblq,
                                      options_.column_split_option ) ),
                          strtk::split_options::compress_delimiters );
        }
...
struct filter_on_wildcard_match {
  public:
    ...
    inline void operator()( const std::pair<Iterator, Iterator>& range ) const {
        if ( match( match_pattern_.begin(), match_pattern_.end(), range.first,
                    range.second, '*', '?' ) ^ allow_through_on_match_ ) {
            predicate_( range );
        }
    }
    ...
    inline void operator()( const std::pair<Iterator, Iterator>& range ) const {
        for ( const std::string* itr = begin_; end_ != itr; ++itr ) {
            if ( ( case_insensitive_ &&
                    ( imatch( ( *itr ).data(), ( *itr ).data() + ( *itr ).size(), range.first,
                              range.second ) ) ) ||
                    ( !case_insensitive_ &&
                      std::equal( ( *itr ).begin(), ( *itr ).end(), range.first ) ) ) {
    ...
    inline void operator()( const std::string& s ) const {
        for ( const std::string* itr = begin_; end_ != itr; ++itr ) {
            if ( ( case_insensitive_ &&
                    ...
                    ( !case_insensitive_ &&
                      std::equal( ( *itr ).begin(), ( *itr ).end(), s.begin() ) ) ) {
                if ( allow_through_on_match_ ) {
                    predicate_( s );
                    return;
                }
...
struct size_equal_to {
    ...
    template <typename T,
              typename Comparator,
              typename Allocator>
    inline bool operator()( const std::multiset<T, Comparator, Allocator>&
                            multiset ) const {
...
inline bool parse( const InputIterator begin,
    ...
    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t1 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t2 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t3 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t4 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t5 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t6 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t7 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t8 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t9 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t10 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t11 ) ) {
        return false;
    }

    ++itr;

    if ( !string_to_type_converter( ( *itr ).first, ( *itr ).second, t12 ) ) {
        return false;
    }
  ...
  public:
    ...
    container_adder( std::vector<T, Allocator>& vec )
        : container_adder_base_( new ( buffer )
                                 sequence_adder_impl<T, Allocator, std::vector>( vec ) )
    {}

    template <typename T, typename Allocator>
    container_adder( std::deque<T, Allocator>& deq )
        : container_adder_base_( new ( buffer )
                                 sequence_adder_impl<T, Allocator, std::deque>( deq ) )
    {}

    template <typename T, typename Allocator>
    container_adder( std::list<T, Allocator>& list )
        : container_adder_base_( new ( buffer )
                                 sequence_adder_impl<T, Allocator, std::list>( list ) )
    {}
    ...
    container_adder( std::priority_queue<T, Container, Comparator>& pq )
        : container_adder_base_( new ( buffer )pq_adder_impl<T, Container, Comparator>
                                 ( pq ) )
    {}
...
template <typename InputIterator,
          ...
          typename Allocator,
          template <typename, typename> class Sequence>
inline std::size_t parse_n( const std::pair<InputIterator, InputIterator>&
                            range,
                            const std::string& delimiters,
                            const std::size_t& n,
                            Sequence<T, Allocator>& sequence,
                            const split_options::type& split_option = split_options::compress_delimiters ) {
    return parse( range.first, range.second, delimiters, n, sequence,
                  split_option );
}
...
inline std::size_t parse_n( const std::pair<InputIterator, InputIterator>&
                            ...
                            std::multiset<T, Comparator, Allocator>& multiset,
                            const split_options::type& split_option = split_options::compress_delimiters ) {
    return parse( range.first, range.second, delimiters, n, multiset,
                  split_option );
}
...
inline std::size_t parse_n( const std::pair<InputIterator, InputIterator>&
                            ...
                            std::priority_queue<T, Container, Comparator>& priority_queue,
                            const split_options::type& split_option = split_options::compress_delimiters ) {
    return parse( range.first, range.second, delimiters, n, priority_queue,
                  split_option );
}
...

{
    ...
    if ( !string_to_type_converter( std::string( argv[0] ), t1 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[1] ), t2 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[2] ), t3 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[3] ), t4 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[4] ), t5 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[5] ), t6 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[6] ), t7 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[7] ), t8 ) ) {
        return result;
    }

    ++result;

    if ( !string_to_type_converter( std::string( argv[8] ), t9 ) ) {
        return result;
    }
...
inline std::string join( const std::string& delimiter, int argc,
                         char* argv[] ) {
    std::string result;
...
template <typename T,
          typename Allocator,
          template <typename, typename> class Sequence>
inline void iota( Sequence<T, Allocator>& sequence, std::size_t count,
                  T value ) {
    while ( count ) {
...
inline void generate_random_values( const std::size_t& count,
    ...
    generate_random_values_impl( count, min, max, std::back_inserter( sequence ),
                                 rng, type );
}
...
inline void random_permutation( const Iterator begin, const Iterator end,
    ...
    boost::variate_generator<boost::mt19937&, boost::uniform_real<double> > rnd(
        rng, dist );
...
inline void random_combination( const Sequence<T, Allocator>& sequence,
                                ...
                                const std::size_t& seed = magic_seed,
                                const std::size_t& pregen = 0 ) {
    random_combination( sequence.begin(), sequence.end(), set_size, out, seed,
                        pregen );
}
...
inline void for_each_combination_conditional( Iterator begin, Iterator end,
    ...
    Iterator mid = begin + size;
    details::bound_range_conditional<Function&, Iterator> func( function, begin,
            mid );
...
inline unsigned long long int n_choose_k( const unsigned long long int& n,
    ...
    class n_choose_k_impl {
      public:
        ...
        inline value_type& lookup( const value_type& n, const value_type& k ) {
            ...
            return table_[static_cast<std::size_t>( ( dimension_ * n ) +
                                                    std::min<value_type>( k, difference ) )];
        }
    ...
    const std::size_t table_size = static_cast<std::size_t>( n * ( n / 2 ) +
                                   ( n & 1 ) );
...
struct numeric_convert_impl<T, Iterator, 19> {
    static inline void process( Iterator itr, T& t ) {
        strtk::fast::details::numeric_convert_impl<T, Iterator, 18>::process( itr + 1,
                t );
...
struct numeric_convert_impl<T, Iterator, 18> {
    static inline void process( Iterator itr, T& t ) {
        strtk::fast::details::numeric_convert_impl<T, Iterator, 17>::process( itr + 1,
                t );
...
inline bool all_digits_check( const std::size_t& n, Iterator itr ) {
    switch ( n ) {
    case  0 :
        return details::all_digits_check_impl<Iterator, 0>::process( itr );

    case  1 :
        return details::all_digits_check_impl<Iterator, 1>::process( itr );

    case  2 :
        return details::all_digits_check_impl<Iterator, 2>::process( itr );

    case  3 :
        return details::all_digits_check_impl<Iterator, 3>::process( itr );

    case  4 :
        return details::all_digits_check_impl<Iterator, 4>::process( itr );

    case  5 :
        return details::all_digits_check_impl<Iterator, 5>::process( itr );

    case  6 :
        return details::all_digits_check_impl<Iterator, 6>::process( itr );

    case  7 :
        return details::all_digits_check_impl<Iterator, 7>::process( itr );

    case  8 :
        return details::all_digits_check_impl<Iterator, 8>::process( itr );

    case  9 :
        return details::all_digits_check_impl<Iterator, 9>::process( itr );

    case 10 :
        return details::all_digits_check_impl<Iterator, 10>::process( itr );

    case 11 :
        return details::all_digits_check_impl<Iterator, 11>::process( itr );

    case 12 :
        return details::all_digits_check_impl<Iterator, 12>::process( itr );

    case 13 :
        return details::all_digits_check_impl<Iterator, 13>::process( itr );

    case 14 :
        return details::all_digits_check_impl<Iterator, 14>::process( itr );

    case 15 :
        return details::all_digits_check_impl<Iterator, 15>::process( itr );

    case 16 :
        return details::all_digits_check_impl<Iterator, 16>::process( itr );

    case 17 :
        return details::all_digits_check_impl<Iterator, 17>::process( itr );

    case 18 :
        return details::all_digits_check_impl<Iterator, 18>::process( itr );

    case 19 :
        return details::all_digits_check_impl<Iterator, 19>::process( itr );
...
inline bool numeric_convert( const std::size_t& n,
    ...
    switch ( n ) {
    case  0 :
        details::numeric_convert_impl<T, Iterator, 0>::process( itr, t );
        return true;

    case  1 :
        details::numeric_convert_impl<T, Iterator, 1>::process( itr, t );
        return true;

    case  2 :
        details::numeric_convert_impl<T, Iterator, 2>::process( itr, t );
        return true;

    case  3 :
        details::numeric_convert_impl<T, Iterator, 3>::process( itr, t );
        return true;

    case  4 :
        details::numeric_convert_impl<T, Iterator, 4>::process( itr, t );
        return true;

    case  5 :
        details::numeric_convert_impl<T, Iterator, 5>::process( itr, t );
        return true;

    case  6 :
        details::numeric_convert_impl<T, Iterator, 6>::process( itr, t );
        return true;

    case  7 :
        details::numeric_convert_impl<T, Iterator, 7>::process( itr, t );
        return true;

    case  8 :
        details::numeric_convert_impl<T, Iterator, 8>::process( itr, t );
        return true;

    case  9 :
        details::numeric_convert_impl<T, Iterator, 9>::process( itr, t );
        return true;

    case 10 :
        details::numeric_convert_impl<T, Iterator, 10>::process( itr, t );
        return true;

    case 11 :
        details::numeric_convert_impl<T, Iterator, 11>::process( itr, t );
        return true;

    case 12 :
        details::numeric_convert_impl<T, Iterator, 12>::process( itr, t );
        return true;

    case 13 :
        details::numeric_convert_impl<T, Iterator, 13>::process( itr, t );
        return true;

    case 14 :
        details::numeric_convert_impl<T, Iterator, 14>::process( itr, t );
        return true;

    case 15 :
        details::numeric_convert_impl<T, Iterator, 15>::process( itr, t );
        return true;

    case 16 :
        details::numeric_convert_impl<T, Iterator, 16>::process( itr, t );
        return true;

    case 17 :
        details::numeric_convert_impl<T, Iterator, 17>::process( itr, t );
        return true;

    case 18 :
        details::numeric_convert_impl<T, Iterator, 18>::process( itr, t );
        return true;

    case 19 :
        details::numeric_convert_impl<T, Iterator, 19>::process( itr, t );
        return true;
...
template <std::size_t N, typename T, typename Iterator>
inline void signed_numeric_convert( Iterator itr, T& t,
                                    const bool digit_check = false ) {
    if ( '-' == ( *itr ) ) {
...
inline bool signed_numeric_convert( const std::string& s,
                                    T& t,
                                    const bool digit_check = false ) {
    return signed_numeric_convert<T, const char*>( s.size(), s.data(), t,
            digit_check );
}
...
static inline unsigned long long int convert( const unsigned long long int v ) {
    ...
    return ( ( v >> 56 ) & 0x00000000000000FFLL ) | ( ( v << 56 ) &
...
static inline unsigned long long int convert( const long long int v ) {
    return static_cast<long long>( convert( static_cast<unsigned long long int>
                                            ( v ) ) );
}
...
class reader {
  public:
    ...
    inline bool operator()( std::ifstream& stream ) {
        ...
        stream.read( original_buffer_,
                     static_cast<std::streamsize>( amount_read_sofar_ ) );
        return true;
    }
    ...
    inline bool operator()( T& output, const std::size_t& size ) {
        if ( buffer_capacity_ok( size ) ) {
            bool result = strtk::string_to_type_converter<char*, T>( buffer_,
                          buffer_ + size, output );
            buffer_ += size;
...
class writer {
  public:
    ...
    inline bool operator()( const std::string& input ) {
        return operator()<const char>( input.data(),
    ...
    template <typename T,
              typename Allocator,
              typename Comparator>
    inline bool operator()( const std::multiset<T, Allocator, Comparator>&
                            multiset ) {
...
class ignore_token {
  public:
    ...
    template <typename InputIterator>
    inline ignore_token& operator=( const
...
class hex_to_number_sink {
  ...
  public:
    ...
    inline hex_to_number_sink& operator=( const
        ...
        if ( !range_only_contains( hex_value_check(), s.first + offset,
                                   s.first + size ) ) {
            valid_ = false;
            return ( *this );
        }
...
class base64_to_number_sink {
  ...
  public:
    ...
    inline base64_to_number_sink& operator=( const std::string& s ) {
        if ( !range_only_contains( base64_value_check(), s.data(),
                                   s.data() + s.size() ) ) {
            valid_ = false;
            return ( *this );
        }
...
inline std::string center( const std::size_t& width,
    ...
    return std::string( pad_count >> 1, pad ) + str + std::string( pad_count_2,
            pad );
}
...
inline void remove_inplace( std::string& s, const std::size_t& index ) {
    ...
    std::memcpy( const_cast<char*>( s.data() + index ),
                 const_cast<char*>( s.data() + ( index + 1 ) ), s.size() - index );
    s.resize( s.size() - 1 );
}
...
inline void insert_inplace( std::string& s, const std::size_t& index,
                            const char c ) {
    s.resize( s.size() + 1 );
    std::memcpy( const_cast<char*>( s.data() + index + 1 ),
                 const_cast<char*>( s.data() + ( index ) ), s.size() - index );
    s[index] = c;
}
...
inline typename details::range_type<Iterator>::type find_n_consecutive(
    ...
    default :
        return typename details::range_type<Iterator>::type( range.second,
                range.second );
    }
}
...
inline std::size_t split_on_consecutive( const std::size_t& n,
    ...
    while ( end != itr_range.first ) {
        range_type found_itr = find_n_consecutive<iterator_type>( n, type, mode,
                               itr_range );
...
inline std::size_t split_on_consecutive_n( const std::size_t& n,
    ...
    while ( ( end != itr_range.first ) && ( match_count <= n ) ) {
        range_type found_itr = find_n_consecutive<iterator_type>( m, type, mode,
                               itr_range );
...
class like_impl {
  public:
    ...
    inline bool operator()( InputIterator begin, InputIterator end ) {
        ...
        return strtk::match( s_.data(), s_.data() + s_.size(), begin, end, zero_or_more,
                             zero_or_one );
    }
...
template <typename T>
inline details::trim_impl<T> trim_leading( const std::string& rem_chars,
        T& t ) {
    return details::trim_impl<T>( 1, t, rem_chars );
}

template <typename T>
inline details::trim_impl<T> trim_trailing( const std::string& rem_chars,
        T& t ) {
...
inline details::fill_array_impl fill_array( char* data,
        const std::size_t& size ) {
    return details::fill_array_impl( reinterpret_cast<unsigned char*>( data ),
                                     size );
}
...
template<> struct numeric<short> {
    enum { length =  5, size = 16, bound_length =  4};
};
template<> struct numeric<unsigned short> {
    enum { length =  5, size = 16, bound_length =  4};
};

template<> struct numeric<int> {
    enum { length = 10, size = 16, bound_length =  9};
};
template<> struct numeric<unsigned int> {
    enum { length = 10, size = 16, bound_length =  9};
};

template<> struct numeric<long> {
    enum { length = 10, size = 16, bound_length =  9};
};
template<> struct numeric<unsigned long> {
    enum { length = 10, size = 16, bound_length =  9};
};
...
template<> struct numeric<unsigned long long int> {
...
template<> struct numeric<long double> {
    enum { min_exp = -308, max_exp = +308, precision = 15};
};
...
template<> struct
    supported_conversion_to_type<strtk::details::fill_array_impl> {
...
template<> struct supported_iterator_type<strtk::details::conv_to_lcase_impl> {
    enum { value = true };
};
...
template<> struct supported_iterator_type<strtk::details::conv_to_ucase_impl> {
    enum { value = true };
};
...
#pragma warning(pop)
        ...
        if ( interim_end != end ) {
            if ( 1 == std::distance( interim_end, end ) ) {
                ...
                static const num_type max               = static_cast<num_type>
...
#pragma warning(pop)
        ...
        if ( interim_end != end ) {
            if ( 1 == std::distance( interim_end, end ) ) {
                ...
                static const num_type max = static_cast<num_type>
                                            ( std::numeric_limits<T>::max() );
                static const num_type min = static_cast<num_type>( static_cast<long long>
                                            ( -1 ) * std::numeric_limits<T>::min() );
                static const num_type positive_penultimate_bound = static_cast<num_type>
                        ( max / 10 );
                static const num_type negative_penultimate_bound = static_cast<num_type>
                        ( min / 10 );
...
template <typename Iterator, typename T>
inline bool string_to_type_converter_impl_ref( Iterator& itr,
        const Iterator end, T& result, signed_type_tag ) {
    if ( end == itr ) {
        return false;
    }
...
template <typename Iterator, typename T>
inline bool parse_inf( Iterator& itr, const Iterator end, T& t,
                       bool negative ) {
...
inline bool type_to_string_converter_impl( T value, std::string& result,
    ...
    itr++;
    result.assign( reinterpret_cast<char*>( itr ),
                   ( buffer + numeric<T>::size ) - itr );
    return true;
}

template <typename T>
inline bool type_to_string_converter_impl( T value, std::string& result,
        strtk::details::signed_type_tag ) {
    static const std::size_t radix = 10;
    static const std::size_t radix_sqr = radix * radix;
    static const std::size_t radix_cube = radix * radix * radix;
...
inline bool type_to_string_converter_impl( const STLContainerType&,
        std::string&, stl_seq_type_tag ) {
...
class ext_string {
  public:
    ...
    inline std::size_t split_n( const DelimiterPredicate& p,
                                ...
                                Sequence<std::string, Allocator>& seq,
                                const split_options::type split_option = split_options::default_mode ) const {
        return strtk::split_n( p, s_, n, range_to_type_back_inserter( seq ),
                               split_option );
    }
    ...
    friend inline ext_string operator * ( const std::size_t& n,
                                          const ext_string& s );
...
inline ext_string operator - ( const ext_string& s,
                               const std::string& pattern ) {
    std::string tmp;
...
inline bool load_file( const std::string& file_name, std::string& buffer ) {
    ...
    return load_file( file_name, const_cast<char*>( buffer.data() ),
                      buffer.size() );
}
...
inline bool write_file( const std::string& file_name,
                        const std::string& buffer ) {
    return write_file( file_name, const_cast<char*>( buffer.data() ),
                       buffer.size() );
}
...
inline bool concatenate( const std::string& file_name1,
    ...
    while ( round < 2 ) {
        ...
        if ( remaining_bytes > 0 ) {
            input_stream.read( &buffer[0],
                               static_cast<std::streamsize>( remaining_bytes ) );
...
inline bool write_pod( std::ofstream& stream,
                       const std::multiset<T, Comparator, Allocator>& multiset ) {
    typename std::multiset<T, Comparator, Allocator>::iterator itr =
        multiset.begin();
...
inline bool read_at_offset( std::ifstream& stream,
    ...
    stream.seekg( static_cast<std::ifstream::off_type>( offset ),
                  std::ios_base::beg );
...
inline unsigned char* read_pod( unsigned char* data,
                                ...
                                T5& t5, T6&   t6, T7&   t7, T8&   t8,
                                T9& t9, T10& t10, T11& t11, T12& t12 ) {
    t1 = ( *reinterpret_cast< T1*>( data ) );
    data += sizeof( T1 );
    t2 = ( *reinterpret_cast< T2*>( data ) );
    data += sizeof( T2 );
    t3 = ( *reinterpret_cast< T3*>( data ) );
    data += sizeof( T3 );
    t4 = ( *reinterpret_cast< T4*>( data ) );
    data += sizeof( T4 );
    t5 = ( *reinterpret_cast< T5*>( data ) );
    data += sizeof( T5 );
    t6 = ( *reinterpret_cast< T6*>( data ) );
    data += sizeof( T6 );
    t7 = ( *reinterpret_cast< T7*>( data ) );
    data += sizeof( T7 );
    t8 = ( *reinterpret_cast< T8*>( data ) );
    data += sizeof( T8 );
    t9 = ( *reinterpret_cast< T9*>( data ) );
    data += sizeof( T9 );
    t10 = ( *reinterpret_cast<T10*>( data ) );
    data += sizeof( T10 );
    t11 = ( *reinterpret_cast<T11*>( data ) );
    data += sizeof( T11 );
    t12 = ( *reinterpret_cast<T12*>( data ) );
    data += sizeof( T12 );
    return data;
}
...
inline unsigned char* write_pod( unsigned char* data,
    ...
    data += sizeof( T1 );
    ( *reinterpret_cast< T2*>( data ) ) =  t2;
    data += sizeof( T2 );
    ( *reinterpret_cast< T3*>( data ) ) =  t3;
    data += sizeof( T3 );
    ( *reinterpret_cast< T4*>( data ) ) =  t4;
    data += sizeof( T4 );
    ( *reinterpret_cast< T5*>( data ) ) =  t5;
    data += sizeof( T5 );
    ( *reinterpret_cast< T6*>( data ) ) =  t6;
    data += sizeof( T6 );
    ( *reinterpret_cast< T7*>( data ) ) =  t7;
    data += sizeof( T7 );
    ( *reinterpret_cast< T8*>( data ) ) =  t8;
    data += sizeof( T8 );
    ( *reinterpret_cast< T9*>( data ) ) =  t9;
    data += sizeof( T9 );
    ( *reinterpret_cast<T10*>( data ) ) = t10;
    data += sizeof( T10 );
    ( *reinterpret_cast<T11*>( data ) ) = t11;
    data += sizeof( T11 );
    ( *reinterpret_cast<T12*>( data ) ) = t12;
    data += sizeof( T12 );
    return data;
}
...
class string_condition {
  private:
    ...
    inline bool condition_notequal( const itr_type begin,
    ...
    inline bool condition_like( const itr_type begin, const itr_type end ) const {
        return match( s_begin, s_end, begin, end, ( unsigned char )'*',
                      ( unsigned char )'?' );
    }
    ...
    inline bool condition_ends_with( const itr_type begin,
                                     const itr_type end ) const {
        if ( s.size() == static_cast<std::size_t>( std::distance( begin, end ) ) ) {
    ...
    inline bool condition_notwithin( const itr_type begin,
                                     const itr_type end ) const {
        if ( s.size() <= static_cast<std::size_t>( std::distance( begin, end ) ) ) {
            return ( end == std::search( begin, end, s_begin, s_end ) );
        }
...
class filter {
  ...
  public:
    ...
    inline bool contains( const std::string& key ) const {
        return contains( reinterpret_cast<const unsigned char*>( key.data() ),
                         key.size() );
    }
...
inline void compute_block( Iterator itr, std::size_t& length,
                           unsigned int& hash ) {
...
template <std::size_t block_size>
inline void compute_block( char* itr, std::size_t& length,
                           unsigned int& hash ) {
    compute_block<block_size>( reinterpret_cast<unsigned char*>( itr ), length,
                               hash );
}
...
class semantic_action_impl {
  ...
  public:
    ...
    template <typename InputIterator>
    inline bool operator()( const std::pair<InputIterator, InputIterator>& r )
    const {
...
template <typename Key,
          ...
          typename OutputIterator>
inline void make_key_list( const std::map<Key, T, Comparator, MapAllocator>&
                           map,
                           OutputIterator out ) {
    if ( map.empty() ) {
        return;
...
template <typename Key,
          ...
          typename MapAllocator,
          typename OutputIterator>
inline void make_value_list( const
                             std::multimap<Key, T, Comparator, MapAllocator>& map,
                             const Key& key,
                             OutputIterator out ) {
    if ( map.empty() ) {
        return;
...
inline void delete_all( std::multimap<Key, T*, Comparator, Allocator>& cont ) {
    typename std::multimap<Key, T*, Comparator, Allocator>::iterator itr =
        cont.begin();
    typename std::multimap<Key, T*, Comparator, Allocator>::iterator end =
        cont.end();
...
template <typename T,
          typename Container,
          typename Comparator>
inline void clear( std::priority_queue<T, Container, Comparator>&
                   priority_queue ) {
...
class column_selector_impl <T0, T1, T2, T3, T4, T5>
  ...
  public:
    ...
    typedef column_selector_base<column_selector_impl<T0, T1, T2, T3, T4, T5>, 6>
    csb_t;
...
inline details::column_list_impl<12> column_list( const std::size_t (
            &idx )[12] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5],
                        idx[6], idx[7], idx[8], idx[9], idx[10], idx[11] );
}

inline details::column_list_impl<11> column_list( const std::size_t (
            &idx )[11] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5],
                        idx[6], idx[7], idx[8], idx[9], idx[10] );
}

inline details::column_list_impl<10> column_list( const std::size_t (
            &idx )[10] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5],
                        idx[6], idx[7], idx[8], idx[9] );
}

inline details::column_list_impl<9> column_list( const std::size_t (
            &idx )[9] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5],
                        idx[6], idx[7], idx[8] );
}

inline details::column_list_impl<8> column_list( const std::size_t (
            &idx )[8] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5],
                        idx[6], idx[7] );
}

inline details::column_list_impl<7> column_list( const std::size_t (
            &idx )[7] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5], idx[6] );
}

inline details::column_list_impl<6> column_list( const std::size_t (
            &idx )[6] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4], idx[5] );
}

inline details::column_list_impl<5> column_list( const std::size_t (
            &idx )[5] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3], idx[4] );
}

inline details::column_list_impl<4> column_list( const std::size_t (
            &idx )[4] ) {
    return column_list( idx[0], idx[1], idx[2], idx[3] );
}

inline details::column_list_impl<3> column_list( const std::size_t (
            &idx )[3] ) {
    return column_list( idx[0], idx[1], idx[2] );
}

inline details::column_list_impl<2> column_list( const std::size_t (
            &idx )[2] ) {
    return column_list( idx[0], idx[1] );
}

inline details::column_list_impl<1> column_list( const std::size_t (
            &idx )[1] ) {
...
struct compose_st_selector_impl <T, 8> {
    ...
    static inline type create( const column_list_t& col_list,
        ...
        typename std::list<T, Allocator>::iterator b = list.begin();
        return type( col_list, *( b ), *inc( b, 1 ), *inc( b, 2 ), *inc( b, 3 ),
                     *inc( b, 4 ), *inc( b, 5 ), *inc( b, 6 ), *inc( b, 7 ) );
    }
};
...
struct compose_st_selector_impl <T, 9> {
    ...
    static inline type create( const column_list_t& col_list,
        ...
        typename std::list<T, Allocator>::iterator b = list.begin();
        return type( col_list, *( b ), *inc( b, 1 ), *inc( b, 2 ), *inc( b, 3 ),
                     *inc( b, 4 ), *inc( b, 5 ), *inc( b, 6 ), *inc( b, 7 ), *inc( b, 8 ) );
    }
};
...
struct memcmp_n_impl {
    ...
    static inline bool process( const unsigned char ( &c1 )[K1],
        ...
        return memcmp_n_impl<N>::process( static_cast<ptr>( c1 ),
                                          static_cast<ptr>( c2 ) );
    }
};
...
inline std::string make_string( const std::pair<const char*, const char*>&
                                range ) {
    return std::string( range.first, range.second );
}

template <typename T, std::size_t N>
inline bool clear_array( T( &a )[N], const T& t,
                         const std::size_t& length = N ) {
    if ( N < length ) {
        return false;
    }
...
inline void fill( std::string& s, const std::string::value_type v ) {
    std::fill( const_cast<char*>( s.data() ),
               const_cast<char*>( s.data() + s.size() ), v );
}
...
class parser {
  public:
    ...
    template <typename T>
    inline bool register_keyvalue( const typename KeyValueMap::key_type& key,
                                   T& t ) {
...
class uintkey_map {
  ...
  public:
    ...
    inline bool operator()( const Range& key_range, const Range& value_range ) {
        ...
        if ( !fast::numeric_convert( distance( key_range ), key_range.first, key,
                                     true ) ) {
            return false;
        }
